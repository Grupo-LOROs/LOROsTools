...
export default function GICotizadorPage() {
  ...
  return (
    <>
      <p className="text-muted mb-4"><a href="/apps">&larr; Apps</a></p>
      <div className="card gi-hero mb-4">
        <div className="gi-kicker">Vista nativa dentro del portal</div>
        <h1>GI - Cotizador ERA/GI</h1>
        <p>Procesa los recibos una sola vez y después ajusta la cotización en tiempo real.</p>
      ...
      <div className="card gi-upload mb-4">
        <div>
          ...
          >
            <div className="gi-drop-title">Arrastra aquí los PDFs o haz clic para seleccionarlos</div>
            <div className="gi-helper">Después de procesarlos, el escenario queda en vivo.</div>
            <input
              ref={inputRef}
              hidden
              type="file"
          ...
          {files.length ? <div className="gi-files">{files.map((file) => <button key={fileKey(file)} type="button" className="gi-file" onClick={() => resetAnalysis(files.filter((item) => fileKey(item) !== fileKey(file)))}>{file.name} <span>×</span></button>)}</div> : null}
        </div>
        <div className="stack-lg">
          <div className="gi-panel-title">Procesamiento</div>
          <div className="gi-status-strip"><div><div className="gi-mini-label">Seleccionados</div><div className="gi-mini-value">{files.length}</div></div><div><div className="gi-mini-label">Procesados</div><div className="gi-mini-value">{receipts.length}</div></div><div><div className="gi-mini-label">Último procesamiento</div><div className="gi-mini-value">{processedAt ? new Date(processedAt).toLocaleTimeString("es-MX") : "Pendiente"}</div></div></div>
          ...
          <div className="gi-helper">{receipts.length ? "Los recibos ya quedaron listos. Ajusta cualquier control y el resultado cambia al instante." : "Primero procesa los PDFs para habilitar la simulación con datos reales."}</div>
        </div>
      </div>
      <div className="gi-layout">
        <div className="gi-sidebar">
          <div className="card">
            ...
            <div className="gi-helper mb-4">Estos controles sí modifican el resultado actual.</div>
            ...
            <SliderField label="Reducción" value={system.reduction_pct} min={0} max={100} step={0.5} hint="Ahorro estimado sobre el recibo." display={`${num(system.reduction_pct, 1)}%`} onChange={(value) => setSystem((current) => ({ ...current, reduction_pct: value }))} />
            ...
            <SliderField label="IVA intereses" value={credit.vat_interest_pct} min={0} max={20} step={0.5} hint="Se usa si está activa la casilla inferior." display={`${num(credit.vat_interest_pct, 1)}%`} onChange={(value) => setCredit((current) => ({ ...current, vat_interest_pct: value }))} />
          ...
          <div className="card">
            ...
            <div className="gi-helper mb-4">Se conservan para captura, pero no mueven el cálculo principal actual.</div>
            <div className="form-group"><label>Meses de instalación</label><input type="number" min={0} max={24} step={1} value={system.months_installation} onChange={(event) => setSystem((current) => ({ ...current, months_installation: toNumber(event.target.value, 0, 24) }))} /></div>
            <div className="form-group"><label>Comisión de apertura (%)</label><input type="number" min={0} max={20} step={0.25} value={credit.opening_fee_pct} onChange={(event) => setCredit((current) => ({ ...current, opening_fee_pct: toNumber(event.target.value, 0, 20) }))} /></div>
            <div className="form-group" style={{ marginBottom: 0 }}><label>IVA comisión (%)</label><input type="number" min={0} max={20} step={0.25} value={credit.vat_on_fee_pct} onChange={(event) => setCredit((current) => ({ ...current, vat_on_fee_pct: toNumber(event.target.value, 0, 20) }))} /></div>
          </div>
        </div>
        <div className="stack-lg">
          {!quote ? <div className="card"><div className="gi-panel-title">Resultado</div><p className="text-muted">Carga los PDFs y presiona <strong>Procesar recibos</strong> para habilitar la cotización en vivo.</p></div> : <>
            <div className="card">
              <div className="flex-between" style={{ gap: 16, flexWrap: "wrap" }}><div><div className="gi-panel-title" style={{ marginBottom: 6 }}>Resultado de la cotización</div><div className="gi-helper">Basado en {receipts.length} recibo(s) procesado(s).</div></div><span className={badgeClass(quote.status)}>{quote.status}</span></div>
              ...
              <div className="gi-meter-head"><strong>Semáforo ICA</strong><span className="gi-helper">Ajustar: 1.00 | Autorizable: 1.10</span></div>
            ...
            <div className="card">
              ...
              <div className="gi-helper mb-4">Haz clic para aplicar una recomendación al escenario actual.</div>
            ...
            <div className="card" style={{ padding: 0, overflow: "hidden" }}>
              <div className="gi-table-head"><div><div className="gi-panel-title" style={{ marginBottom: 6 }}>Recibos procesados</div><div className="gi-helper">Datos extraídos y alertas detectadas por PDF.</div></div></div>
              <div className="gi-table-wrap"><table><thead><tr><th>Archivo</th><th>Período</th><th>kWh total</th><th>Base recomendada</th><th>Facturación</th><th>DAP</th><th>Total</th><th>Confianza</th><th>Alertas</th></tr></thead><tbody>{receipts.map((receipt) => <tr key={receipt.file}><td>{receipt.file}</td><td>{receipt.period_raw || "-"}</td><td>{receipt.kwh_total != null ? num(receipt.kwh_total, 0) : "-"}</td><td>{receipt.base_amount_recommended != null ? money(receipt.base_amount_recommended) : "-"}</td><td>{receipt.billing_period != null ? money(receipt.billing_period) : "-"}</td><td>{receipt.dap != null ? money(receipt.dap) : "-"}</td><td>{receipt.total_to_pay != null ? money(receipt.total_to_pay) : "-"}</td><td><span className={badgeClass(receipt.confidence)}>{receipt.confidence === "high" ? "Alta" : receipt.confidence === "medium" ? "Media" : "Baja"}</span></td><td>{receipt.warnings.length ? receipt.warnings.join("; ") : "Sin alertas"}</td></tr>)}</tbody></table></div>
            </div>
            <div className="card" style={{ padding: 0, overflow: "hidden" }}>
              <div className="gi-table-head"><div><div className="gi-panel-title" style={{ marginBottom: 6 }}>Amortización</div><div className="gi-helper">Primeros 24 meses del escenario actual.</div></div></div>
              <div className="gi-table-wrap"><table><thead><tr><th>Mes</th><th>Saldo inicial</th><th>Pago</th><th>Interés</th><th>IVA interés</th><th>Principal</th><th>Saldo final</th></tr></thead><tbody>{schedule.slice(0, 24).map((row) => <tr key={row.month}><td>{row.month}</td><td>{money(row.balance_start)}</td><td>{money(row.payment)}</td><td>{money(row.interest)}</td><td>{money(row.vat_interest)}</td><td>{money(row.principal)}</td><td>{money(row.balance_end)}</td></tr>)}</tbody></table></div>
            </div>
          </>}
        </div>
      </div>
