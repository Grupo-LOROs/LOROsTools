...
async def analyze_bank_movements(
    ...
    if not app or not app.enabled:
        raise HTTPException(status_code=404, detail="La herramienta de Tesoreria no esta disponible.")
    ...
    for item in valid_files:
        if Path(item.filename or "").suffix.lower() != ".pdf":
            raise HTTPException(status_code=400, detail=f"Solo se permiten archivos PDF. Recibi: {item.filename}")
